...
def extract_pipeline(docs, max_workers=4):
    filtered_docs = [x for x in docs if x is not None]
...
final = final.with_columns(
    ...
    .map_elements(
        ...
        return_dtype=pl.Utf8  # Adjust this to match summarizer's return type
    )
    .alias("summary")
)
...
representation_model = DSPyRepresentation(nr_docs=25)
...
topic_model = BERTopic(representation_model=representation_model, embedding_model=embedding_model, vectorizer_model=vectorizer_model, umap_model=umap_model, hdbscan_model=cluster_model)
...
data = {
    ...
    for k, v in topic_model.topic_labels_.items()
}
