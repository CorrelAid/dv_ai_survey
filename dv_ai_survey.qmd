...
df_nna_q3 <- df %>%
  ...
  filter(!is.na(Q9)) %>%
  mutate(
    Q9_binary = if_else(Q9 > 50, TRUE, FALSE)  # Create Q9_binary
  ) %>%
  left_join(labels, by = "Q3")  # Add Q3_label to the dataset

cor_result <- cor.test(as.numeric(df_nna_q3$Q3), as.numeric(df_nna_q3$Q9_binary), method = "pearson")
...
ggplot(df_nna_q3, aes(x = Q9)) + 
  geom_density(aes(fill = Q3_label), color = dv_black, alpha = 0.5) + 
  scale_fill_colorblind(name = "KI Nutzung") +  
  scale_color_colorblind(name = "Q3") +        
  theme(
  ...
  annotate("text", x = 50, y = 0.02, 
           ...
           color = dv_black, size = 5, hjust = 0) +


  annotate("text", x = 0, y = -0.0005, label = "Hype", color = dv_black) +
  annotate("text", x = 98, y = -0.0005, label = "Mehrwert", color = dv_black) +

  geom_vline(data = group_means, aes(xintercept = mean_Q9, color = Q3_label), 
             linetype = "dashed", show.legend = FALSE) + 

  geom_text(data = group_means, aes(x = mean_Q9, y = 0.015, 
           label = paste0("Ø (", Q3_label, "): ", round(mean_Q9, 1))), 
           size = 4, hjust = -0.1, show.legend = FALSE) + 

  labs(
    title = str_wrap("KI Nutzung vs Gesamteinschätzung", 75),
...
Q4_Options_map$label_clean <- clean_options(Q4_Options_map$label)
...
response_counts <- df_long %>%
  ...
  left_join(Q4_Options_map, by = "question")
...
Q16_Options_map$label_clean <- clean_options(Q16_Options_map$label)
...
ggplot(response_counts, aes(x = forcats::fct_reorder(label_clean, Percentage), y = Percentage)) +
  geom_bar(stat = "identity", fill = dv_dark_blue) +
  labs(
    title = str_wrap(paste(Q16_Text, " (>5%)"), width = 60),
    x = NULL,
    y = "Prozent der befragten Organisationen"
  ) +
  coord_flip() +
...
df_long_q3 <- df %>%
  select(Q3, starts_with("Q16")) %>%
  pivot_longer(cols = starts_with("Q16"), 
               names_to = "question", 
               values_to = "response") %>%
  filter(!is.na(response) & !is.na(Q3)) %>%  
  group_by(Q3, question) %>%
  summarise(
    Total = n(),  
    Yes = sum(response == TRUE) 
  ) %>%
...
ggplot(df_long_q3, aes(x = forcats::fct_reorder(label_clean, Percentage), y = Percentage, fill = Q3)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = str_wrap("Q3: Aktive KI-Nutzung vs. Q16: Nutzung von KI-Anwendungen in Vereinsarbeit", width = 60),
    x = NULL,
    y = "Prozent der befragten Organisationen"
  ) +
  coord_flip() +
...
selected_columns <- df %>% select(starts_with("Q8")) %>% select(-Q8_open)
...
df_opps %>% 
  ...
  geom_text(data = df_opps %>% distinct(label, mean_value),
            aes(x = mean_value, y = Inf, label = sprintf("Ø %.1f", mean_value)),
            vjust = 2, hjust = -0.7, color = "red", inherit.aes = FALSE) +
